...
function tokensFromMeta(meta) {
  if (!meta || typeof meta !== 'object') return { in: 0, out: 0, cost: 0, source: 'observed' };
  ...
  if ('contextTokensUsed' in meta) {
    return {
      in: Number(meta.contextTokensUsed) || 0,
      out: 0,
      cost: 0,
      source: 'estimated',
    };
  ...
  return { in: 0, out: 0, cost: 0, source: 'observed' };
}

function mergeSource(current, next) {
  const rank = { none: 0, observed: 1, estimated: 2, reported: 3 };
  return (rank[next] || 0) > (rank[current] || 0) ? next : current;
}

function aggregate(sessionsRoot, providerIds = []) {
  const providers = {};
  const rateLimitTs = {};

  for (const id of providerIds) {
    if (id) providers[id] = emptyProvider();
  }
    ...
    for (const file of files) {
      ...
      for (const line of content.split('\n')) {
        ...
        if (ev.type === 'usage') {
          ...
          p.calls += 1;
          const { in: tin, out: tout, cost, source } = tokensFromMeta(ev.meta);
          p.tokensIn += tin;
          p.tokensOut += tout;
          p.costUsd += cost;
          p.source = mergeSource(p.source, source);
