...
function memberLabel(member, index) {
  return `${index}: ${member.adapter}${member.model ? ' - ' + member.model : ''}${member.role ? ' (' + member.role + ')' : ''}`;
}
...
async function runPlan({ adapters, buildContext, store, team, prompt, ws, sid, cwd, signal }) {
  ...
  const fullPrompt = buildContext(ws, sid, planPrompt);
  const chunks = [];
...
async function runApproved({ adapters, buildContext, store, team, steps, ws, sid, cwd, signal }) {
  ...
  for (let i = 0; i < plan.length; i++) {
    ...
    try {
      const fullPrompt = buildContext(ws, sid, step.task);
      await adapters.run(
        member.adapter,
