...
const SKIP_DIRS = new Set([
  ...
  'coverage', '.nyc_output', '.venv', 'venv', 'env', '.cache', 'vendor',
  'sessions', 'logs', 'tmp', '.turbo', '.parcel-cache',
]);
...
async function runPlan({ adapters, buildContext, store, team, prompt, ws, sid, cwd, signal }) {
  ...
  const planPrompt = makePlanPrompt(team, prompt, fileTree);
...
async function runApproved({ adapters, buildContext, store, team, steps, ws, sid, cwd, signal }) {
  ...
  for (let i = 0; i < plan.length; i++) {
    ...
    try {
      ...
      await adapters.run(
        member.adapter,
        { prompt: fullPrompt, model: member.model, effort: member.effort, cwd, mode, signal },
        (e) =>
          store.append(ws, sid, {
            kind: 'agent',
            actor: member.adapter,
